...
if (!state.city) {
cta.innerHTML = user
? `<button class="home-cta-btn home-cta-primary" onclick="openProfile()">👤 ${(user.user_metadata?.full_name || user.email).split(' ')[0]}'s Profile</button> <a class="home-cta-btn home-cta-sec" href="business-landing.html">For Business</a>`
: `<button class="home-cta-btn home-cta-primary" onclick="openAuth('signin')">Sign In / Sign Up</button> <a class="home-cta-btn home-cta-sec" href="business-landing.html">For Business</a>`;
}
}
...
</div>`: ''} <div class="s-secondary-actions"> ${v.url ?`<a class="s-act-btn s-act-primary" href="${v.url}" target="_blank" rel="noopener" onclick="event.stopPropagation()">Website ↗</a>`:`<a class="s-act-btn s-act-primary" href="https://www.google.com/search?q=${encodeURIComponent(v.name + ' ' + (state.city?.name || 'San Diego'))}" target="_blank" rel="noopener" onclick="event.stopPropagation()">Google ↗</a>`} <button class="s-act-btn" onclick="goToMap('${v.id}')">Map</button> <button class="s-act-btn" onclick="shareItem('${v.id}','${type}')">Share</button> ${isVenue ? `<button class="s-act-btn" id="venue-follow-btn-${v.id}" onclick="toggleVenueFollow('${v.id}','${esc(v.name)}',this)">Follow</button>`: ''} </div> ${isVenue && (v.instagram || v.facebook || v.twitter || v.tiktok || v.phone) ?`
<div class="s-social-row">
${v.instagram ? `<a class="s-social-btn s-social-ig" href="https://instagram.com/${v.instagram.replace(/^@/,'')}" target="_blank" rel="noopener" onclick="event.stopPropagation()" title="Instagram"> <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><circle cx="12" cy="12" r="3"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg> <span>Instagram</span></a>` : ‘’}
${v.tiktok ? `<a class="s-social-btn s-social-tt" href="https://tiktok.com/@${v.tiktok.replace(/^@/,'')}" target="_blank" rel="noopener" onclick="event.stopPropagation()" title="TikTok"> <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.18 8.18 0 0 0 4.78 1.52V6.76a4.85 4.85 0 0 1-1.01-.07z"/></svg> <span>TikTok</span></a>` : ‘’}
${v.facebook ? `<a class="s-social-btn s-social-fb" href="${v.facebook.startsWith('http') ? v.facebook : 'https://facebook.com/' + v.facebook}" target="_blank" rel="noopener" onclick="event.stopPropagation()" title="Facebook"> <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg> <span>Facebook</span></a>` : ‘’}
${v.twitter ? `<a class="s-social-btn s-social-tw" href="https://twitter.com/${v.twitter.replace(/^@/,'')}" target="_blank" rel="noopener" onclick="event.stopPropagation()" title="X / Twitter"> <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg> <span>X</span></a>` : ‘’}
${v.phone ? `<a class="s-social-btn s-social-ph" href="tel:${v.phone}" onclick="event.stopPropagation()" title="Call"> <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.27h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.91a16 16 0 0 0 6.06 6.06l.92-.92a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg> <span>Call</span></a>` : ‘’}
...
document.getElementById(‘authContent’).innerHTML = `<div class="auth-title">${si ? 'Welcome back' : 'Create account'}</div> <p class="auth-sub">${si ? 'Sign in to save spots & manage reviews' : 'Free forever — save spots, write reviews'}</p> ${!si ?`<div class="field-group"><div class="field-label">Name</div><input class="field" id="aName" type="text" placeholder="Your name" autocomplete="name"></div>`: ''} <div class="field-group"><div class="field-label">Email</div><input class="field" id="aEmail" type="email" placeholder="you@example.com" autocomplete="email"></div> <div class="field-group"><div class="field-label">Password</div><input class="field" id="aPass" type="password" placeholder="${si ? 'Your password' : 'Min 8 characters'}" autocomplete="${si ? 'current-password' : 'new-password'}"></div> ${si ?`<button class="auth-forgot" onclick="doForgot()">Forgot password?</button>` : ''} <button class="btn-submit" id="authBtn" onclick="doAuth('${mode}')" style="width:100%;margin-top:4px">${si ? 'Sign In' : 'Create Account'}</button> <p class="auth-switch">${si ? "No account?" : 'Have an account?'} <button class="auth-switch-btn" onclick="renderAuth('${si ? 'signup' : 'signin'}')">${si ? 'Sign up free' : 'Sign in'}</button></p>`;
setTimeout(() => {
...
document.getElementById(‘profileContent’).innerHTML = `<div class="my-profile-header"> <div class="my-avatar-wrap"> <div class="my-avatar" id="myAvatar" onclick="toggleAvatarPicker()" title="Change avatar">${avatar}</div> <div class="avatar-picker" id="avatarPicker" style="display:none"> ${AVATARS.map(e =>`<button class="avatar-opt" onclick="pickAvatar('${e}',this)">${e}</button>`).join('')} </div> </div> <div class="my-name">${esc(displayName)}</div> <div class="profile-email">${esc(user.email)}</div> <div class="my-stats" id="myStatBar"> <div class="my-stat" onclick="openActivityFeed()" style="cursor:pointer"><span>${checkIns.length}</span>Check-ins</div> <div class="my-stat"><span>${myReviews.length}</span>Reviews</div> ${currentStreak >= 2 ? `<div class="my-stat streak-stat"><span>${currentStreak}🔥</span>Streak</div>`: ''} <div class="my-stat" onclick="openFindPeople()" style="cursor:pointer"><span id="stat-following">${following.length}</span>Following</div> <div class="my-stat" style="cursor:pointer"><span id="stat-followers">${followers.length}</span>Followers</div> </div> </div> ${badges.length ?`<div class="pub-badges">${badges.map(b => {
...
el.innerHTML = ` <div class="photo-prompt-title">Add a photo? 📸</div> <div class="photo-prompt-sub">Show others what's happening at ${esc(venueName)} right now.</div> <div class="photo-upload-area" id="photoUploadArea" ondragover="event.preventDefault();this.classList.add('dragover')" ondragleave="this.classList.remove('dragover')" ondrop="handlePhotoDropOrChange(event,'${venueId}','${esc(venueName)}')"> <input type="file" accept="image/*" capture="environment" onchange="handlePhotoDropOrChange(event,'${venueId}','${esc(venueName)}')"> <div class="photo-upload-icon">📷</div> <div class="photo-upload-hint">Tap to take a photo or<br><strong>choose from your library</strong></div> </div> <div class="photo-preview-wrap" id="photoPreviewWrap"> <img id="photoPreviewImg" src="" alt="Preview"> <button class="photo-preview-remove" onclick="clearPhotoPreview()">✕</button> </div> <textarea class="photo-caption-field" id="photoCaptionField" placeholder="Add a caption (optional)…" rows="2"></textarea> <button class="photo-submit-btn" id="photoSubmitBtn" disabled onclick="submitPhotoCheckin('${venueId}','${esc(venueName)}')">Share Photo</button> <button class="photo-skip-btn" onclick="skipToTagFriends('${venueId}')">Skip →</button>`;
