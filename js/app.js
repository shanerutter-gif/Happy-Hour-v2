...
bar.innerHTML = ` <button class="bottom-nav-btn active" id="bnFeed" onclick="bottomNavFeed(this)"> <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg> <span>Feed</span> </button> <button class="bottom-nav-btn" id="bnSocial" onclick="bottomNavSocial(this)"> <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> <span>Social</span> </button> <button class="bottom-nav-btn" id="bnMessages" onclick="bottomNavMessages(this)"> <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> <span>Messages<span class="bn-badge" id="bnMsgBadge" style="display:none"></span></span> </button> <button class="bottom-nav-btn" id="bnProfile" onclick="bottomNavProfile(this)"> <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg> <span id="bnProfileLabel">${(user.user_metadata?.full_name || 'Profile').split(' ')[0]}</span> </button>`;
...
overlay.innerHTML = `<div style="background:var(--card);border-radius:20px;padding:28px 24px;max-width:320px;width:100%;text-align:center;box-shadow:0 20px 60px rgba(42,31,20,0.18);animation:scaleInModal .22s ease"> <div style="font-size:32px;margin-bottom:10px">📸</div> <div style="font-family:'Cabinet Grotesk',sans-serif;font-size:19px;font-weight:900;margin-bottom:8px;color:var(--text)">Share your night out</div> <div style="font-size:13px;color:var(--muted);line-height:1.5;margin-bottom:20px">Check in at a spot and add a photo — it shows up in this feed for everyone in the city.</div> <button onclick="document.getElementById('socialNudgeOverlay').remove()" style="width:100%;padding:13px;background:var(--coral);color:#fff;border:none;border-radius:12px;font-family:'Cabinet Grotesk',sans-serif;font-size:15px;font-weight:700;cursor:pointer">Got it 👊</button> ${seen < 2 ?`<div style="font-size:11px;color:var(--muted);margin-top:8px">${2 - seen} reminder${2 - seen !== 1 ? ‘s’ : ‘’} left</div>` : ''} </div>`;
...
if (item.type === ‘photo’) {
return `<div class="social-card social-card--photo"> <div class="social-card-header"> <div class="social-avatar" ${profileClick}>${avatar}</div> <div class="social-card-meta"> <div class="social-card-name" ${profileClick}>${esc(displayName)}${followBadge}</div> <div class="social-card-action">checked in at <span class="social-venue-link" ${venueClick}>${esc(venueName)}</span></div> <div class="social-card-time">${neighborhood ? neighborhood + ' · ' : ''}${timeAgo}</div> </div> </div> <div class="social-photo-wrap" ${venueClick}> <img class="social-photo" src="${esc(item.photo_url)}" alt="${esc(venueName)}" loading="lazy" onerror="this.closest('.social-card').remove()"> </div> ${item.caption ? `<div class="social-caption">${esc(item.caption)}</div>` : ''} </div>`;
}

// ── Check-in (no photo) ──
if (item.type === ‘check_in’) {
return `<div class="social-row" ${venueClick ? venueClick.replace('onclick=', 'onclick=') : ''}> <div class="social-avatar" ${profileClick}>${avatar}</div> <div class="social-row-body"> <div class="social-row-text"> <span class="social-row-name" ${profileClick}>${esc(displayName)}</span>${followBadge} checked in at <span class="social-venue-link" ${venueClick}>${esc(venueName)}</span> </div> <div class="social-row-meta">${neighborhood ? neighborhood + ' · ' : ''}${timeAgo}</div> ${item.meta?.note ? `<div class="social-row-note">”${esc(item.meta.note)}”</div>` : ''} </div> <div class="social-row-icon">📍</div> </div>`;
}

// ── Review ──
if (item.type === ‘review’) {
const stars = item.meta?.rating ? ‘★’.repeat(item.meta.rating) + ‘☆’.repeat(5 - item.meta.rating) : ‘’;
return `<div class="social-row" ${venueClick}> <div class="social-avatar" ${profileClick}>${avatar}</div> <div class="social-row-body"> <div class="social-row-text"> <span class="social-row-name" ${profileClick}>${esc(displayName)}</span>${followBadge} reviewed <span class="social-venue-link" ${venueClick}>${esc(venueName)}</span> </div> ${stars ? `<div class="social-row-stars">${stars}</div>`: ''} ${item.meta?.text ?`<div class="social-row-note">”${esc(item.meta.text)}”</div>` : ''} <div class="social-row-meta">${neighborhood ? neighborhood + ' · ' : ''}${timeAgo}</div> </div> <div class="social-row-icon">⭐</div> </div>`;
}

// ── Favorite ──
if (item.type === ‘favorite’) {
return `<div class="social-row" ${venueClick}> <div class="social-avatar" ${profileClick}>${avatar}</div> <div class="social-row-body"> <div class="social-row-text"> <span class="social-row-name" ${profileClick}>${esc(displayName)}</span>${followBadge} saved <span class="social-venue-link" ${venueClick}>${esc(venueName)}</span> </div> <div class="social-row-meta">${neighborhood ? neighborhood + ' · ' : ''}${timeAgo}</div> </div> <div class="social-row-icon">🔖</div> </div>`;
}

// ── Going tonight ──
if (item.type === ‘going_tonight’) {
return `<div class="social-row" ${venueClick}> <div class="social-avatar" ${profileClick}>${avatar}</div> <div class="social-row-body"> <div class="social-row-text"> <span class="social-row-name" ${profileClick}>${esc(displayName)}</span>${followBadge} is going to <span class="social-venue-link" ${venueClick}>${esc(venueName)}</span> tonight </div> <div class="social-row-meta">${neighborhood ? neighborhood + ' · ' : ''}${timeAgo}</div> </div> <div class="social-row-icon">🔥</div> </div>`;
}
...
grid.innerHTML = `<div class="empty-state"> <svg width="96" height="96" viewBox="0 0 96 96" fill="none" xmlns="http://www.w3.org/2000/svg"> <circle cx="48" cy="48" r="40" fill="rgba(255,107,74,0.08)"/> <circle cx="44" cy="44" r="18" stroke="#FF6B4A" stroke-width="3" fill="none"/> <line x1="57" y1="57" x2="70" y2="70" stroke="#FF6B4A" stroke-width="3" stroke-linecap="round"/> <circle cx="44" cy="38" r="3" fill="rgba(255,107,74,0.4)"/> <path d="M37 47 Q44 54 51 47" stroke="rgba(255,107,74,0.4)" stroke-width="2" stroke-linecap="round" fill="none"/> </svg> <div class="empty-state-title">No spots found</div> <div class="empty-state-sub">Try removing a filter or searching something different</div> <button class="request-venue-btn request-venue-btn--empty" onclick="openRequestVenue()">+ Request a Venue</button> </div>`;
return;
}
...
const photoBlock = hasPhoto ? `<div class="card-photo-wrap"> <img src="${photoUrl}" class="card-photo-img" alt="${esc(v.name)}" loading="lazy" onerror="this.closest('.card-photo-wrap').style.display='none';this.closest('.card').classList.add('card-no-photo')"> <div class="card-photo-gradient"></div> <div class="card-photo-name-over">${esc(v.name)}</div> ${distBadge ?`<div class="card-photo-dist-badge">${distBadge}</div>` : ''} <button class="card-photo-heart${faved ? ' faved' : ''}" onclick="event.stopPropagation();doFavorite('${v.id}','venue',this);this.classList.toggle('faved');this.textContent=this.classList.contains('faved')?'★':'☆'">${faved ? '★' : '☆'}</button> </div>` : ‘’;
return `<div class="card${hasPhoto ? '' : ' card-no-photo'}" data-id="${v.id}" onclick="openModal('${v.id}','venue')" role="button" tabindex="0"> ${photoBlock} <div class="card-body"> ${hasPhoto ? '' : `<div class="card-top">
...
</div>`} <div class="card-meta"> <span>${esc(v.neighborhood || '')}</span> ${v.neighborhood && v.hours ? '<span class="card-sep">·</span>' : ''} ${v.hours ? `<span class="card-when">${esc(getTodayHours(v))}</span>`: ''} ${!hasPhoto && distBadge ?`<span class="card-sep">·</span><span class="card-dist">${distBadge}</span>`: ''} </div> ${v.featured ? '<div class="featured-crown">⭐ Featured</div>' : ''} ${(() => { const tags = AMENITIES.filter(a => v[a.key]).map(a =>`<span class="amenity-tag amenity-tag--${a.key}">${a.emoji} ${a.label}</span>`).join(''); return tags ? `<div class="amenity-tags">${tags}</div>`: ''; })()} <ul class="deals">${(v.deals || []).slice(0, 3).map(d =>`<li>${esc(d)}</li>`).join('')}${(v.deals || []).length > 3 ? `<li class="deals-more">+${v.deals.length - 3} more</li>`: ''}</ul> ${goingFireBadge(v.id)} </div> <div class="card-foot"> <span class="card-cuisine">${esc(v.cuisine || '')}${v.owner_verified ? ' <span class="verified-badge verified-badge--card">✓ Verified</span>' : ''}</span> <div class="card-stars">${starHTML(avg, 5, 11)}<span class="card-rcount">${cached.length ?`(${cached.length})`: '—'}</span></div> </div> <div class="card-going"> <button class="going-btn${state.goingByMe.has(v.id) ? ' going-active' : ''}" onclick="event.stopPropagation();doGoingTonight('${v.id}',this)">${checkInBtnLabel(state.goingCounts[v.id]||0, state.goingByMe.has(v.id))}</button> </div> ${!v.owner_verified ?`<div class="card-claim"><a href="business-portal.html" onclick="event.stopPropagation()" class="claim-link">Own this spot? Claim it →</a></div>` : ‘’}
...
const faved = isFavorite(v.id);
return `<div class="card" onclick="openModal('${v.id}','event')" role="button" tabindex="0"> <div class="card-top"> <div class="card-name">${esc(v.name)}</div> <button class="heart-btn${faved ? ' faved' : ''}" onclick="event.stopPropagation();doFavorite('${v.id}','event',this)">${faved ? '★' : '☆'}</button> </div> <div class="card-meta"> <span>${esc(v.neighborhood || '')}</span> ${v.neighborhood ? '<span class="card-sep">·</span>' : ''} <span class="card-when">${esc(v.hours || '')}</span> </div> ${v.description ? `<ul class="deals"><li>${esc(v.description)}</li></ul>` : ‘’}
<div class="card-foot">
...
document.getElementById(‘modalContent’).innerHTML = `${(() => { const photos = v.photo_urls?.length ? v.photo_urls : (v.photo_url ? [v.photo_url] : []); if (!photos.length) return ''; if (photos.length === 1) { return`<div class="s-photo-thumb" onclick="openPhotoLightbox('${photos[0]}','${esc(v.name)}')" title="Tap to enlarge"><img src="${photos[0]}" alt="${esc(v.name)}" loading="lazy" onerror="this.parentElement.remove()"><div class="s-photo-hint">📷 Tap to expand</div></div>`; } return `<div class="s-photos-strip">${photos.map((url, i) =>
`<div class="s-photo-thumb s-photo-thumb--multi" onclick="openPhotoLightbox('${url}','${esc(v.name)}')" title="Tap to enlarge"> <img src="${url}" alt="${esc(v.name)} photo ${i+1}" loading="lazy" onerror="this.parentElement.remove()"> <div class="s-photo-hint">📷 ${i+1}/${photos.length}</div> </div>`
).join(’’)}</div>`; })()} <div class="s-tag ${isVenue ? 'hh' : 'ev'}">${isVenue ? '🍺 Happy Hour' : esc(v.event_type || 'Event')}</div> <div style="display:flex;align-items:flex-start;gap:10px;padding-right:38px"> <div style="flex:1"> <div class="s-name">${esc(v.name)}${v.owner_verified ? ' <span class="verified-badge verified-badge--modal">✓ Verified</span>' : ''}</div> <div class="s-hood">${esc(v.neighborhood || '')}</div> <div class="s-addr">📍 ${esc(v.address || '')}</div> ${isVenue ? (() => { const tags = AMENITIES.filter(a => v[a.key]).map(a => `<span class="amenity-tag amenity-tag--${a.key}">${a.emoji} ${a.label}</span>`).join(''); return tags ? `<div class="amenity-tags amenity-tags--modal">${tags}</div>`: ''; })() : ''} </div> <button class="heart-btn heart-btn--lg${faved ? ' faved' : ''}" onclick="doFavorite('${v.id}','${type}',this)" style="margin-top:4px">${faved ? '★' : '☆'}</button> </div> <div class="s-div"></div> <div class="s-label">Schedule</div> <div class="s-when">${esc(v.hours || '')}</div> <div class="s-days">${DAYS.map(d =>`<span class="day-pill${(v.days || []).includes(d) ? (d === TODAY ? ' today' : ' on') : ''}">${d}</span>`).join('')}</div> ${isVenue ? `
<div class="s-div"></div>
...
return `<div class="s-div"></div> <div class="s-label">Events at this venue</div> <div class="s-events-list">${evs.map(e => { const evToday = (e.days||[]).includes(TODAY); return `<div class="s-event-item">
...
</div>`: ''} <div class="s-secondary-actions"> ${v.url ?`<a class="s-act-btn s-act-primary" href="${v.url}" target="_blank" rel="noopener" onclick="event.stopPropagation()" title="Website"><span class="s-btn-icon">🌐</span></a>`:`<a class="s-act-btn s-act-primary" href="https://www.google.com/search?q=${encodeURIComponent(v.name + ' ' + (state.city?.name || 'San Diego'))}" target="_blank" rel="noopener" onclick="event.stopPropagation()" title="Search"><span class="s-btn-icon">🔍</span></a>`} <button class="s-act-btn" onclick="goToMap('${v.id}')" title="Map"><span class="s-btn-icon">🗺️</span></button> <button class="s-act-btn" onclick="shareItem('${v.id}','${type}')" title="Share"><span class="s-btn-icon">↗️</span></button> ${currentUser ? `<button class="s-act-btn" onclick="dmOpenVenueSharePicker('${v.id}')" title="Send"><span class="s-btn-icon">💬</span></button>`: ''} ${isVenue ?`<button class="s-act-btn" id="venue-follow-btn-${v.id}" onclick="toggleVenueFollow('${v.id}','${esc(v.name)}',this)" title="Follow"><span class="s-btn-icon">🔔</span></button>`: ''} </div> ${isVenue ?`<div id="ugc-photos-${v.id}"></div>`: ''} <div class="s-div"></div> <div class="s-label">Reviews <span id="ravg-${v.id}">${avgHTML(reviews)}</span></div> <div class="review-form"> <div class="star-picker" id="sp-${v.id}" data-val="0">${[1,2,3,4,5].map(n =>`<button class="sp" onclick="pickStar('${v.id}',${n})">★</button>`).join('')}</div> ${!currentUser ? `<p class="review-guest-note">Posting as guest — <button class="auth-switch-btn" onclick="openAuth('signin')">sign in</button> to manage reviews</p>` : ''} <input class="field" id="rname-${v.id}" type="text" value="${currentUser ? esc(currentUser.user_metadata?.full_name || '') : ''}" placeholder="Your name" ${currentUser ? 'style="display:none"' : ''} autocomplete="name"> <textarea class="field" id="rtext-${v.id}" placeholder="How was it?" rows="3"></textarea> <button class="btn-submit" onclick="submitReview('${v.id}','${type}')">Post Review</button> </div> <div class="reviews-list" id="rlist-${v.id}">${reviews.length ? renderReviewList(reviews, v.id, type) : '<div class="no-reviews">Loading…</div>'}</div>`;
}
...
return `<div class="review-item"> <div class="review-head"> ${nameEl} <span class="review-stars">${starHTML(r.rating, 5, 11)}</span> <span class="review-date">${fmtDate(r.created_at)}</span> </div> ${r.text ? `<div class="review-text">${esc(r.text)}</div>`: ''} ${isOwn ?`<div class="review-acts">
...
document.getElementById(‘editContent’).innerHTML = `<div class="s-name" style="font-size:20px">Edit Review</div> <div class="review-form" style="margin-top:14px"> <div class="star-picker" id="epick" data-val="${rating}">${[1,2,3,4,5].map((n,i) =>`<button class="sp${i<rating?' lit':''}" onclick="pickEditStar(${n})">★</button>`).join('')}</div> <textarea class="field" id="etext" rows="4">${esc(text)}</textarea> <div style="display:flex;gap:8px"> <button class="btn-submit" style="flex:1" onclick="saveEditReview('${reviewId}','${itemId}','${type}')">Save</button> <button class="btn-sec" onclick="closeOverlay('editOverlay')">Cancel</button> </div> </div>`;
...
document.getElementById(‘authContent’).innerHTML = ` <div class="auth-title">Set new password</div> <p class="auth-sub">Choose a strong password for your account</p> <div class="field-group"> <div class="field-label">New Password</div> <input class="field" id="rPass1" type="password" placeholder="Min 8 characters" autocomplete="new-password"> </div> <div class="field-group"> <div class="field-label">Confirm Password</div> <input class="field" id="rPass2" type="password" placeholder="Repeat password" autocomplete="new-password"> </div> <button class="btn-submit" id="resetBtn" onclick="doResetPassword()" style="width:100%;margin-top:4px">Update Password</button>`;
...
document.getElementById(‘profileContent’).innerHTML = `<!-- ── DARK HEADER ── --> <div class="pf-hero" id="myBanner"> <div class="pf-burst"></div> <div class="pf-grid"></div> <div class="pf-ring pf-r1"></div> <div class="pf-ring pf-r2"></div> <div class="pf-ring pf-r3"></div> <div class="pf-actions"> <button class="pf-btn" onclick="shareSpotd()" title="Share"> <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg> </button> <button class="pf-btn" onclick="openProfileSettings()" title="Settings"> <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg> </button> </div> <div class="pf-avatar-zone"> <div class="pf-avatar" id="myAvatar" onclick="toggleAvatarPicker()" title="Change avatar">${avatar}</div> <div class="avatar-picker" id="avatarPicker" style="display:none"> ${AVATARS.map(e =>`<button class="avatar-opt" onclick="pickAvatar('${e}',this)">${e}</button>`).join(’’)}
</div>
</div>
</div>
...
overlay.onclick = e => { if (e.target === overlay) dismissOverlay(overlay); };
overlay.innerHTML = ` <div style="background:var(--card);border-radius:20px;padding:28px 24px;max-width:300px;width:90%;text-align:center;position:relative;"> <button onclick="dismissOverlay(this.closest('.overlay'))" style="position:absolute;top:12px;right:16px;background:none;border:none;font-size:20px;cursor:pointer;color:var(--muted);">✕</button> <div style="font-size:48px;margin-bottom:12px;">${def.emoji || '🏅'}</div> <div style="font-size:18px;font-weight:800;margin-bottom:8px;">${def.label || badgeKey}</div> <div style="font-size:14px;color:var(--muted);line-height:1.5;">${def.desc || 'Badge earned on Spotd'}</div> <div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border);font-size:12px;color:var(--muted);">🏆 You've earned this badge!</div> </div>`;
document.body.appendChild(overlay);
}
...
content.innerHTML = (profiles || []).map(p => `<div style="display:flex;align-items:center;gap:12px;padding:14px 20px;border-bottom:1px solid var(--border);cursor:pointer;" onclick="closeSubPage('followersPage');openPublicProfile('${p.id}')"> <div style="width:42px;height:42px;border-radius:50%;background:var(--bg2);display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;">${p.avatar_emoji || '🍺'}</div> <div style="flex:1;min-width:0;"> <div style="font-weight:700;font-size:14px;">${esc(p.display_name || 'Spotd User')}</div> ${p.username ?`<div style="font-size:12px;color:var(--muted);">@${esc(p.username)}</div>` : ''} </div> <div style="color:var(--muted);font-size:18px;">›</div> </div>`).join(’’) || ‘<div style="text-align:center;padding:32px;color:var(--muted)">No followers yet</div>’;
}
...
if (!currentUser) { openAuth(‘signin’); return; }
document.getElementById(‘findPeopleContent’).innerHTML = ` <div style="position:relative;margin-bottom:16px"> <input class="field" id="peopleSearch" type="text" placeholder="Search by name…" oninput="debouncePeopleSearch(this.value)" style="width:100%;box-sizing:border-box"> </div> <div id="peopleResults"><div style="text-align:center;padding:32px;color:var(--muted)">Loading…</div></div>`;
...
return `<div class="people-row"> <div class="feed-avatar" onclick="closeSubPage('findPeoplePage');openPublicProfile('${p.id}')" style="cursor:pointer">${p.avatar_emoji || '🍺'}</div> <div class="people-info" onclick="closeSubPage('findPeoplePage');openPublicProfile('${p.id}')" style="cursor:pointer;flex:1;min-width:0"> <div class="people-name">${esc(name)}</div> ${p.bio ? `<div class="people-bio">${esc(p.bio)}</div>` : ‘’}
</div>
...
lb.innerHTML = `<button onclick="this.parentElement.remove()" style="position:absolute;top:16px;right:16px;background:rgba(255,255,255,0.15);border:none;color:#fff;font-size:22px;width:40px;height:40px;border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;">✕</button> <img src="${url}" alt="${name}" style="max-width:100%;max-height:90vh;object-fit:contain;border-radius:8px;box-shadow:0 8px 40px rgba(0,0,0,0.5);">`;
...
document.getElementById(‘pubProfileContent’).innerHTML = `<div class="pub-profile-header"> <div class="pub-avatar">${avatar}</div> <div class="pub-profile-info"> <div class="pub-name">${esc(displayName)}</div> ${profile.username ?`<div class="pub-username">@${esc(profile.username)}</div>`: ''} ${profile.bio ?`<div class="pub-bio">${esc(profile.bio)}</div>`: ''} <div class="pub-stats"> <div class="pub-stat"><span>${checkIns.length}</span>Check-ins</div> <div class="pub-stat"><span>${reviews.length}</span>Reviews</div> <div class="pub-stat"><span>${totalVenues}</span>Venues</div> </div> </div> </div> ${badges.length ?`<div class="pub-badges">${badges.map(b => {
const def = BADGE_DEFS[b.badge_key] || {};
return `<span class="badge-chip" title="${def.desc || b.badge_key}">${def.emoji || '🏅'} ${def.label || b.badge_key}</span>`;
}).join(’’)}</div>`: ''} ${currentUser && currentUser.id !== userId ?`
...
</div>`: ''} <div class="pub-tabs"> <button class="pub-tab active" onclick="switchPubTab('checkins', this)">📍 Check-ins</button> <button class="pub-tab" onclick="switchPubTab('reviews', this)">⭐ Reviews</button> <button class="pub-tab" onclick="switchPubTab('favorites', this)">♥ Saved</button> </div> <div id="pub-tab-checkins" class="pub-tab-content active"> ${recentCheckIns.length ? recentCheckIns.map(c => { const v = allItems.find(x => String(x.id) === String(c.venue_id)); return`<div class="pub-activity-row" onclick="${v ? `closeOverlay('pubProfileOverlay');openModal('${c.venue_id}','venue')` : ''}">
...
</div>`; }).join('') : '<div class="pub-empty">No check-ins yet</div>'} </div> <div id="pub-tab-reviews" class="pub-tab-content" style="display:none"> ${reviews.length ? reviews.map(r => { const item = allItems.find(x => String(x.id) === String(r.venue_id || r.event_id)); return `<div class="pub-activity-row" onclick="${item ? `closeOverlay('pubProfileOverlay');openModal('${r.venue_id||r.event_id}','${r.venue_id?'venue':'event'}')` : ''}">
<div class="pub-activity-icon">⭐</div>
<div class="pub-activity-body">
...
</div>`; }).join('') : '<div class="pub-empty">No reviews yet</div>'} </div> <div id="pub-tab-favorites" class="pub-tab-content" style="display:none"> ${favSpots.length ? favSpots.map(v => `
...
</div>`).join('') : '<div class="pub-empty">No saved spots</div>'} </div>`;
}
...
if (!currentUser) { openAuth(‘signin’); return; }
document.getElementById(‘feedContent’).innerHTML = ` <div class="feed-header"> <button class="feed-tab-btn active" id="ftab-following" onclick="switchFeedTab('following',this)">Following</button> <button class="feed-tab-btn" id="ftab-mine" onclick="switchFeedTab('mine',this)">My Activity</button> </div> <div id="feedRows"><div style="text-align:center;padding:40px;color:var(--muted)">Loading…</div></div>`;
...
const following = await getFollowing(currentUser.id);
if (!following.length) {
document.getElementById(‘feedRows’).innerHTML = ` <div class="pub-empty" style="padding:40px 16px"> <div style="font-size:32px;margin-bottom:12px">👋</div> <div style="font-weight:600;margin-bottom:8px">No activity yet</div> <div style="color:var(--muted);font-size:13px">Follow people to see their check-ins & reviews here</div> <button class="profile-action-btn" style="margin-top:16px;max-width:180px" onclick="closeSubPage('feedPage');openFindPeople()">🔍 Find People</button> </div>`;
return;
}
...
document.getElementById(‘requestContent’).innerHTML = `<div class="s-tag hh">Request a Venue</div> <div class="s-name" style="font-size:22px;margin-bottom:4px">Know a great spot?</div> <p style="font-size:13px;color:var(--muted);margin-bottom:20px;line-height:1.6">Tell us about a venue you'd like to see on Spotd. We'll review it and add it if it's a fit.</p> <div class="field-group"> <div class="field-label">Venue name *</div> <input class="field" id="req-name" type="text" placeholder="e.g. The Tipsy Crow" autocomplete="off"> </div> <div class="field-group"> <div class="field-label">Neighborhood</div> ${areas.length ?`<select class="field" id="req-neighborhood">
<option value="">Select a neighborhood…</option>
${areas.map(a => `<option value="${esc(a)}">${esc(a)}</option>`).join(’’)}
<option value="__other__">Other / Not sure</option>
</select>`:`<input class="field" id="req-neighborhood" type="text" placeholder="e.g. Gaslamp Quarter">`} </div> <div class="field-group"> <div class="field-label">Why should we add it?</div> <textarea class="field" id="req-reason" rows="3" placeholder="Great happy hour deals, hidden gem, friend works there..."></textarea> </div> <div id="req-msg" style="display:none;font-size:13px;padding:10px 14px;border-radius:8px;margin-bottom:12px"></div> <button class="btn-submit" id="req-btn" onclick="submitVenueRequest()" style="width:100%">Submit Request</button> <p style="font-size:11px;color:var(--muted);text-align:center;margin-top:12px">We review all requests manually — usually within a few days.</p>`;
...
const neighborhood = rawNeighborhood === ‘**other**’ ? ‘’ : rawNeighborhood;
...
document.getElementById(‘requestContent’).innerHTML = `<div style="text-align:center;padding:32px 16px"> <div style="font-size:48px;margin-bottom:16px">🙌</div> <div class="s-name" style="font-size:22px;margin-bottom:8px">Request sent!</div> <p style="font-size:14px;color:var(--muted);line-height:1.6">Thanks for the tip. We'll review <strong style="color:var(--text)">${esc(name)}</strong> and add it to Spotd if it's a great fit.</p> <button class="btn-sec" style="margin:24px auto 0;display:flex" onclick="closeOverlay('requestOverlay')">Close</button> </div>`;
}
...
el.innerHTML = `<div class="tag-prompt-title">Who'd you go with?</div> <div class="tag-prompt-sub">Tag a friend at ${esc(venueName)} and they'll see it in their feed.</div> <div class="tag-friends-grid" id="tagFriendsGrid"> <div style="color:var(--muted);font-size:13px">Loading friends…</div> </div> <button class="tag-skip-btn" onclick="closeOverlay('tagFriendsOverlay')">Skip</button>`;
...
return `<div class="s-div"></div> <div class="ugc-photos-section"> <div class="ugc-photos-label">📸 From the crowd <span style="font-weight:400;font-size:10px">${photos.length} photo${photos.length !== 1 ? 's' : ''}</span></div> <div class="ugc-photos-strip"> ${photos.map(p =>`
...
const uploadZone = isNative ? ` <div class="photo-upload-area" id="photoUploadArea"> <div class="photo-upload-icon">📷</div> <div class="photo-source-btns"> <button class="photo-source-btn" onclick="window._capacitorTakePhoto().catch(e=>{ if(e.message!=='User cancelled photos app') showToast('Camera unavailable'); })"> 📷 Take Photo </button> <button class="photo-source-btn" onclick="window._capacitorChoosePhoto().catch(e=>{ if(e.message!=='User cancelled photos app') showToast('Could not open library'); })"> 🖼️ Choose from Library </button> </div> </div>` : ` <div class="photo-upload-area" id="photoUploadArea" ondragover="event.preventDefault();this.classList.add('dragover')" ondragleave="this.classList.remove('dragover')" ondrop="handlePhotoDropOrChange(event,'${venueId}','${esc(venueName)}')"> <input type="file" accept="image/*" id="photoFileInput" style="position:absolute;inset:0;width:100%;height:100%;opacity:0;cursor:pointer;z-index:2" onchange="handlePhotoDropOrChange(event,'${venueId}','${esc(venueName)}')"> <div class="photo-upload-icon" style="position:relative;z-index:1;pointer-events:none">📷</div> <div class="photo-upload-hint" style="position:relative;z-index:1;pointer-events:none"> Tap to add a photo </div> </div>`;

el.innerHTML = ` <div class="photo-prompt-title">Add a photo? 📸</div> <div class="photo-prompt-sub">Show others what's happening at ${esc(venueName)} right now.</div> ${uploadZone} <div class="photo-preview-wrap" id="photoPreviewWrap"> <img id="photoPreviewImg" src="" alt="Preview"> <button class="photo-preview-remove" onclick="clearPhotoPreview('${venueId}','${esc(venueName)}')">✕</button> </div> <textarea class="photo-caption-field" id="photoCaptionField" placeholder="Add a caption (optional)…" rows="2"></textarea> <button class="photo-submit-btn" id="photoSubmitBtn" disabled onclick="submitPhotoCheckin('${venueId}','${esc(venueName)}')">Share Photo</button> <button class="photo-skip-btn" onclick="skipToTagFriends('${venueId}')">Skip →</button>`;
...
bar.innerHTML = `<div class="dm-members-pills">${sorted.map(p => `<div class="dm-member-pill">${p.avatar_emoji||‘🍺’} ${esc((p.display_name||‘User’).split(’ ‘)[0])}${p.id===currentUser.id?’ (you)’:’’}</div>` ).join('')}</div>`;
};
...
return `<div class="dm-msg ${cls}">${groupLabel} <div class="dm-venue-card" onclick="openModal('${m.venue_id}','venue')"> <div class="dm-venue-icon">📍</div> <div class="dm-venue-info"> <div class="dm-venue-name">${esc(v.name || 'Venue')}</div> <div class="dm-venue-meta">${esc(v.neighborhood || '')}${v.google_rating ? ` · ⭐ ${v.google_rating}` : ''}</div> </div> <div class="dm-venue-arrow">›</div> </div>${time}</div>`;
}
...
tmp.innerHTML = `<div class="dm-bubble">${esc(body)}</div> <div class="dm-msg-time">${new Date().toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'})}</div>`;
el?.appendChild(tmp);
dmScrollToBottom();
...
document.getElementById(‘dmPickerScroll’).innerHTML = `<div class="dm-picker-search-wrap"> <input class="dm-picker-search" placeholder="Search by name…" oninput="dmFilterPicker(this.value)"> </div> ${isGroup ?`<div class="dm-group-name-wrap"><input class="dm-picker-search" id="dmGroupName" placeholder="Group name (optional)…"></div>`:`<div class="dm-picker-toggle" onclick="dmShowPicker(window._dmPickerUsers,true)">👥 Create Group Instead</div>`} <div id="dmPickerList"> ${users.length ? users.map(u =>`
...
</div>`).join('') : '<div class="dm-empty">Follow people to message them</div>' } </div>`;
}
...
overlay.onclick = e => { if (e.target === overlay) dismissOverlay(overlay); };
overlay.innerHTML = `<div class="sheet" style="max-height:60vh;overflow-y:auto;"> <div style="font-weight:800;font-size:17px;margin-bottom:16px;padding-right:32px;">Send to…</div> <button class="sheet-close" onclick="document.getElementById('dmSharePickerOverlay').remove()">✕</button> ${uniqueConvos.map(c => { const others = convoPartsMap[c.id] || []; const myFirst = (currentUser?.user_metadata?.full_name || 'You').split(' ')[0]; const name   = c.is_group ? (c.name || [myFirst,...others.map(id=>(pMap[id]?.display_name||'User').split(' ')[0])].join(', ')) : (pMap[others[0]]?.display_name || 'Spotd User'); const avatar = c.is_group ? '👥' : (pMap[others[0]]?.avatar_emoji || '🍺'); return `<div class="dm-thread-row" style="border-bottom:1px solid var(--bg2);" onclick="dmSendVenue('${venueId}','${c.id}');document.getElementById('dmSharePickerOverlay').remove()">
