...
r.innerHTML = user
? `${bizLink} <button class="nav-btn" onclick="openFavView()">★ Saved</button> <button class="nav-btn nav-profile" onclick="openProfile()">${(user.user_metadata?.full_name || user.email).split(' ')[0]} ↗</button> <button class="nav-btn nav-signout" onclick="doSignOut()">Sign out</button>`
: `${bizLink} <button class="nav-btn nav-login" onclick="openAuth('signin')">Sign In / Join</button>`;
}
...
grid.innerHTML = `<div class="no-results"> No venues match — try different filters <div style="margin-top:16px"> <button class="request-venue-btn request-venue-btn--empty" onclick="openRequestVenue()">+ Request a Venue</button> </div> </div>`;
return;
}
...
? `<div class="card-event-pills">${attachedEvents.slice(0,3).map(e => { const evToday = (e.days||[]).includes(TODAY); return `<span class="card-event-pill${evToday ? ' card-event-pill--today' : ''}">🎉 ${esc(e.event_type||‘Event’)} · ${(e.days||[]).slice(0,2).join(’/’)}${evToday ? ’ · Tonight’ : ‘’}</span>`; }).join('')}</div>`
: ‘’;
return `<div class="card" data-id="${v.id}" onclick="openModal('${v.id}','venue')" role="button" tabindex="0"> <div class="card-top"> <div class="card-name">${esc(v.name)}</div> <div class="card-right"> <button class="heart-btn${faved ? ' faved' : ''}" onclick="event.stopPropagation();doFavorite('${v.id}','venue',this)">${faved ? '★' : '☆'}</button> <div class="card-badge ${isToday ? 'today' : 'dim'}">${isToday ? 'Today' : 'Open'}</div> </div> </div> <div class="card-meta"> <span>${esc(v.neighborhood || '')}</span> ${v.neighborhood ? '<span class="card-sep">·</span>' : ''} <span class="card-when">${esc(v.hours || '')}</span> ${state.sort === 'distance' && state.userLat !== null ? '<span class="card-sep">·</span><span class="card-dist">' + fmtDistance(haversine(state.userLat, state.userLng, v.lat, v.lng)) + '</span>' : ''} </div> ${v.featured ? '<div class="featured-crown">⭐ Featured</div>' : ''} ${(() => { const tags = AMENITIES.filter(a => v[a.key]).map(a => `<span class="amenity-tag amenity-tag--${a.key}">${a.emoji} ${a.label}</span>`).join(''); return tags ? `<div class="amenity-tags">${tags}</div>`: ''; })()} <ul class="deals">${(v.deals || []).slice(0, 3).map(d =>`<li>${esc(d)}</li>`).join('')}${(v.deals || []).length > 3 ? `<li class="deals-more">+${v.deals.length - 3} more</li>`: ''}</ul> ${eventPillsHTML} ${goingFireBadge(v.id)} <div class="card-foot"> <span class="card-cuisine">${esc(v.cuisine || '')}</span> <div class="card-stars">${starHTML(avg, 5, 11)}<span class="card-rcount">${cached.length ?`(${cached.length})`: '—'}</span></div> </div> <div class="card-going"> <button class="going-btn${state.goingByMe.has(v.id) ? ' going-active' : ''}" onclick="event.stopPropagation();doGoingTonight('${v.id}',this)">${checkInBtnLabel(state.goingCounts[v.id]||0, state.goingByMe.has(v.id))}</button> </div> ${!v.owner_verified ?`<div class="card-claim"><a href="business-portal.html" onclick="event.stopPropagation()" class="claim-link">Own this spot? Claim it →</a></div>` : ‘’}
...
const isToday = (v.days || []).includes(TODAY);
const faved   = isFavorite(v.id);
return `<div class="card" onclick="openModal('${v.id}','event')" role="button" tabindex="0"> <div class="card-top"> <div class="card-name">${esc(v.name)}</div> <div class="card-right"> <button class="heart-btn${faved ? ' faved' : ''}" onclick="event.stopPropagation();doFavorite('${v.id}','event',this)">${faved ? '★' : '☆'}</button> <div class="card-badge event-badge event-badge--${(v.event_type||'event').toLowerCase().replace(/\s+/g,'-')}">${EVENT_TYPE_AMENITY[v.event_type]?.emoji || '🎉'} ${esc(v.event_type || 'Event')}</div> </div> </div> <div class="card-meta"> <span>${esc(v.neighborhood || '')}</span> ${v.neighborhood ? '<span class="card-sep">·</span>' : ''} <span class="card-when">${esc(v.hours || '')}</span> ${isToday ? '<span class="card-sep">·</span><span style="color:var(--teal);font-size:11px">Tonight</span>' : ''} </div> ${v.description ? `<ul class="deals"><li>${esc(v.description)}</li></ul>` : ‘’}
<div class="card-foot">
...
`} <div class="s-div"></div> <div class="s-actions"> ${v.url ? `<a class="btn-primary" href="${v.url}" target="_blank" rel="noopener" onclick="event.stopPropagation()">Website ↗</a>`:`<a class="btn-primary btn-secondary" href="https://www.google.com/search?q=${encodeURIComponent(v.name + ' ' + (state.city?.name || 'San Diego'))}" target="_blank" rel="noopener" onclick="event.stopPropagation()">Google ↗</a>`} <button class="btn-sec" onclick="goToMap('${v.id}')">Map</button> <button class="btn-sec" onclick="shareItem('${v.id}','${type}')">Share</button> </div> ${isVenue ? `
...
document.getElementById(‘findPeopleContent’).innerHTML = ` <div class="s-name" style="font-size:20px;margin-bottom:12px">Find People</div> <div style="position:relative;margin-bottom:16px"> <input class="field" id="peopleSearch" type="text" placeholder="Search by name…" oninput="debouncePeopleSearch(this.value)" style="width:100%;box-sizing:border-box"> </div> <div id="peopleResults"></div>`;
...
return `<div class="people-row"> <div class="feed-avatar" onclick="closeOverlay('findPeopleOverlay');openPublicProfile('${p.id}')" style="cursor:pointer">${p.avatar_emoji || '🍺'}</div> <div class="people-info" onclick="closeOverlay('findPeopleOverlay');openPublicProfile('${p.id}')" style="cursor:pointer;flex:1;min-width:0"> <div class="people-name">${esc(name)}</div> ${p.bio ? `<div class="people-bio">${esc(p.bio)}</div>` : ‘’}
</div>
