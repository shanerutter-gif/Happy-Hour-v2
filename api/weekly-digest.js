...
// every subscribed contact in that city (matched on the `city_slug` contact
// property). The email body itself lives in Loops — we only send the event +
// properties (cityName + 3-5 spot names/deals).
...
export default async function handler(req) {
  ...
  try {
    ...
    // 3. Fetch all Loops contacts (paginated) and bucket by city_slug property.
    let allContacts = [];
    let hasMore = true;
    let loopOffset = 0;
    while (hasMore && loopOffset < 10000) {
      const lr = await fetch(`https://app.loops.so/api/v1/contacts?limit=100&offset=${loopOffset}`, {
        headers: { 'Authorization': `Bearer ${loopsKey}` },
      });
      if (!lr.ok) {
        const body = await lr.text();
        console.error('[weekly-digest] Loops contacts fetch failed:', lr.status, body);
        break;
      }
      const batch = await lr.json();
    ...
    // 4. Send weekly_digest to each contact whose city_slug we have a digest for.
    const perCity = {};
    let sent = 0, errors = 0;
    for (const contact of allContacts) {
      if (!contact.email) continue;
      const slug = contact.city_slug;
      const digest = slug && cityProps[slug];
      if (!digest) continue;
      try {
        ...
        if (er.ok) { sent++; perCity[slug] = (perCity[slug] || 0) + 1; }
        else {
          errors++;
          const body = await er.text();
          console.error(`[weekly-digest] weekly_digest send failed for ${contact.email}:`, er.status, body);
        }
      } catch (e) {
        errors++;
        console.error(`[weekly-digest] weekly_digest send error for ${contact.email}:`, e.message);
      }
